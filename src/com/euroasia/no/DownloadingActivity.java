...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
		...
		headerview.setText("Nedlastninger");
        ...
        if(totaldownloading >= 1){
        	text.setText("Totale nedlastninger: "+totaldownloading);
        }else{
        	text.setText("Totale nedlastninger: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Nullstilt Fullført & Mislykte Nedlastninger");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Nedlastning venter");
		    		}else if(a == 0){
		    			progress.setText("Nedlastning fullført");
		    		}else if(a == -1){
		        		progress.setText("Nedlastning mislyktes");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Nedlastningsvalg")
				       .setCancelable(false)
				       .setNegativeButton("Fjern", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 Nedlastning slettet");
				           }
				       })
				       .setNeutralButton("Prøv igjen", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Sett til prøv igjen");
				           }
				       })
				...
				final CharSequence[] items = {"Åpne MP3", "Sett som ringetone", "Last ned igjen", "Slett", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Velg et alternativ");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Åpne MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Feil under lasting av MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Sett som ringetone"){
				    	...
				    	}else if(items[item] == "Slett"){
							AlertDialog.Builder builder = new AlertDialog.Builder(DownloadingActivity.this);
							builder.setMessage("Er du sikker på at du vil slette: " + title + " ?")
							       .setCancelable(false)
							       .setPositiveButton("Ja", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 slettet");
							           }
							       })
							       .setNegativeButton("Nei", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 ikke slettet");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Last ned igjen"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Sett til prøv igjen");
				    	}
				...
				builder.setMessage("Nedlastningsvalg")
				       ...
				       .setNegativeButton("Fjern", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 nedlastnning slettet");
				           }
				       })
				...
				builder.setMessage("Nedlastningsvalg")
				       .setCancelable(false)
				       .setNegativeButton("Stopp nedlastning", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("MP3 nedlastning stoppet");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Laster ned nå", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Nedlastning venter", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Fullførte nedlastninger", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Nedlastning mislyktes", 100));
