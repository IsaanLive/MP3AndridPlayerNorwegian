...
public class SearchActivity extends ListActivity {
    ...
    public void startsearch(){
		...
		toastmake("S¿ker etter: "+searchstring);
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("S¿k har blitt avbrutt");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("S¿ker etter sanger...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Det later til at du ikke har en aktiv Internettforbindelse eller har et svakt signal. Vennligst aktiver WiFi eller Data og pr¿v igjen.");
		        }else{
					failresults.setText("Ingen treff funnet");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Det oppsto en feil under s¿ket. Vennligst pr¿v igjen");
		        }
			}else{
				setListAdapter(adapter);
			}
